...
240 AZMX-generated brand images, exported from the `Gradient & Bks` page of the New Direction Library Figma file. All are AI-generated by the AZMX design team (Midjourney and Seedream), so they are cleared for AZMX use with no third-party stock licensing.

Format: JPG, 1600 px wide, quality 70. Total 53 MB. The files are not stored in this repository: they live in the separate `azmx-brand-cdn` repository and are served from the jsDelivr CDN at `https://cdn.jsdelivr.net/gh/Gamaleldientarek/azmx-brand-cdn@main/images/<section>/<file>.jpg`, for example `https://cdn.jsdelivr.net/gh/Gamaleldientarek/azmx-brand-cdn@main/images/blue/blue-014.jpg`. Files are numbered per section. `references/image-index.md` lists every file with its direct link; `scripts/image-meta.json` is the machine-readable catalogue (dominant colour, nearest token, luminance) the gallery and index are built from.

To embed an image in a document, download it first: `curl -L -o blue-014.jpg "https://cdn.jsdelivr.net/gh/Gamaleldientarek/azmx-brand-cdn@main/images/blue/blue-014.jpg"`. In HTML or email, use the CDN URL directly.

## Sections

| Section | CDN folder | Count | Use for |
|---|---|---|---|
| Gradients | `gradient/` | 34 | Navy-to-electric gradient fields. Cover backgrounds, dividers, closings, event surfaces. |
| Abstract Blue | `blue/` | 112 | The core brand imagery set: abstract 3D forms, architecture, textures in navy and electric blue. Default choice for any AZMX surface needing an image. |
| Orange | `orange/` | 28 | Categorical or client-specific work only. Not a brand color. |
| Purple | `purple/` | 24 | Categorical or client-specific work only. Not a brand color. |
| Green | `green/` | 11 | Categorical or client-specific work only. Not a brand color. |
| Yellow | `yellow/` | 4 | Categorical or client-specific work only. Not a brand color. |
| Red | `red/` | 22 | Categorical or client-specific work only. Not a brand color. |
| White | `white/` | 5 | Light editorial surfaces, abstract compositions on white. |

## Colour pairing (measured, not assumed)

Every image was analysed for dominant colour and luminance. The headline finding: **all but the White section are dark surfaces** (relative luminance below 0.18). Treat an image background as a Navy surface unless it comes from the White section.
...
2. Compare against what is already catalogued in `scripts/image-meta.json` (or `references/image-index.md`) so existing images are not duplicated. Section counts at last export: gradient 34, blue 112, orange 28, purple 24, green 11, yellow 4, red 22, white 5.
...
5. Compress each file to JPEG quality 70 — `python3 scripts/add-images.py <section> <folder>` does this with Pillow, or on macOS `sips -s format jpeg -s formatOptions 70` gives the same result. Figma exports at very high quality: the first full export was 204 MB and compressed to 46 MB with no visible artefacts, checked on a gradient-heavy sample.
6. Run `python3 scripts/add-images.py <section> <folder>` with the `azmx-brand-cdn` checkout next to this repo (or `--cdn-dir PATH`): it numbers the files after the highest entry in `scripts/image-meta.json`, writes them into `<cdn-dir>/images/<section>/`, appends their analysis to `scripts/image-meta.json`, and rebuilds the index and gallery. Commit and push both repositories. The gallery updates a minute or two after pushing; jsDelivr serves new files from `main` within about 12 hours (purge a path via `https://purge.jsdelivr.net/gh/Gamaleldientarek/azmx-brand-cdn@main/images/<section>/<file>` to force it).
