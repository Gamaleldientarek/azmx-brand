...
- **Token CSS is usable as-is** — `azmx-tokens.css` now carries units decided by each token's primitive (`--azmx-space-md: 64px`, `--azmx-type-body-md-size: 18px`), Figma weight names become numeric weights (`Regular` → `400`), font families are quoted, the hand-written stacks are `--azmx-font-stack-heading/-text`, and the 37 RTL tokens are emitted under `:root, [dir="ltr"]` / `[dir="rtl"]`. The generator validates before emitting and refuses duplicate names. `tokens-to-css.mjs` is now an importable module (`resolve`, `resolveAll`, `loadTokens`, `cssValue`…); the explorer shares its resolver.
- **One rule set for tone** — `extract-metrics.py` imports its palette/CSS parsing and tone rules from `brand-check.py` instead of carrying a diverged copy; the intensifier and hedging lists were reconciled into one superset (`elevate`, `unlock`, `delve`, `effortlessly` and the phrase forms are now flagged by both tools). `drift-detector.py`'s four near-identical analyzers are one `analyze_metric()`.
- **`brand-check.py --brand`** — unknown names exit 2 and list the available brands; `typography_overrides` from `config/sub-brands/*.json` are honoured (Majarah's Oswald is no longer a blocker); configs are validated against `schemas/sub-brand-config.schema.json`; `_example-*.json` is not selectable. `check_file` is split into per-section helpers (no behaviour change); the HASHTAG line number is now the line of the hashtag.
- **`add-images.py`** uses Pillow instead of macOS `sips` (works on every platform, reports the real error on failure).
- **Image library moved to a CDN** — the 240 JPEGs (53 MB) now live in the separate `Gamaleldientarek/azmx-brand-cdn` repository and are served from jsDelivr at `https://cdn.jsdelivr.net/gh/Gamaleldientarek/azmx-brand-cdn@main/images/<section>/<file>.jpg`, so a skill install drops from ~60 MB to ~8 MB. This repository keeps the catalogue, `scripts/image-meta.json` (per-image dominant colour, nearest token, luminance, and the CDN base as `$meta.cdn`); `rebuild-index.py` builds `index.html` and `references/image-index.md` from it without any image file present (a local `assets/images/` copy, now gitignored, is re-measured if it exists). `add-images.py` writes new JPEGs into the CDN checkout (`--cdn-dir`, default `../azmx-brand-cdn`), numbers them from the catalogue, and appends their analysis. The gallery, the index, `api/v1/images.json` (now with a `cdn` base and a per-image `url`), `api/v1/index.json` and the docs all link to the CDN; old `gamaleldientarek.github.io/azmx-brand/assets/images/...` URLs are forwarded to the CDN by a new root `404.html`. `SKILL.md` and `AZMX-Figma-SKILL.md` tell agents to download an image (`curl -L -o <file> <cdn url>`) before embedding it in a document, and to use `references/image-index.md` as the index of what exists.
